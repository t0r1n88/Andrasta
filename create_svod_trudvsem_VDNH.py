...
def vdnh_processing_data_trudvsem(file_data:str,file_org:str,end_folder:str,region:str):
    ...
    try:
        ...
        if len(company_df) != 0:
            ...
            json_folder = f'{end_folder}/JSON по организациям/{current_date}'
            ...
            for idx, row in enumerate(company_df.itertuples()):
                ...
                if len(temp_df) != 0:
                    temp_df.sort_values(by=['Минимальная зарплата'], inplace=True,ascending=False)  # сортируем
                    ...
                    # Создаем JSON
                    if not os.path.exists(f'{json_folder}/{name_company}'):
                        os.makedirs(f'{json_folder}/{name_company}')

                    temp_json_df = temp_df[['Вакансия','Полное название работодателя','Зарплата']]
                    temp_json_df.to_json(f'{json_folder}/{name_company}/{name_company[:15]}.json')

                    # Создаем QR коды
                    if not os.path.exists(f'{qr_folder}/{name_company}'):
                        os.makedirs(f'{qr_folder}/{name_company}')
                    ...
                    for row in temp_df.itertuples():
                        ...
                        if os.path.isfile(f'{qr_folder}/{name_company}/{id_qr}.png'):
                            # если такой файл есть то добавляем постфикс в виде индекса строки
                            img.save(f'{qr_folder}/{name_company}/{id_qr}_{row[0]}.png')
                        else:
                            img.save(f'{qr_folder}/{name_company}/{id_qr}.png')
...
if __name__ == '__main__':
    ...
    main_org_file = 'data/Пример топ вакансий.xlsx'

    main_region = 'Республика Бурятия'



    main_end_folder = 'c:/Users/1/PycharmProjects/Andrasta/data/Республика Бурятия'
